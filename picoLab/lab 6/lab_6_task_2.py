'''
this function does not use any circuits.

the program first calls the GetFileData function which opens the thermData.txt file in read mode and then uses a
for loop to get each line individually. it splits the line into 2 data points using .split and converts them into
ints and floats. it then appends them / adds them to the end of the x and y arrays.
one its done with all the lines it closes the file and returns the x and y point arrays.

the second function call is PlotData which takes in a x and y array and creates a graph using them by plotting the
x and y points and then adds information to the graph like a title or label before drawing the graph.
'''
